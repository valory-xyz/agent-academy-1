...
import numpy as np
import mock_node.grpc_client as grpc_client
...
class GrpcTests(TestCase):
    def setUp(self) -> None:
        self.grpc = grpc_client.GrpcClient()

    @given(message=text())
    def test_echo(self, message: str):
        assert self.grpc.echo(message)
    ...
    def test_info(self, version: str, block_version: int, p2p_version: int):
        assert self.grpc.info(version, block_version, p2p_version)

    def test_flush(self):
        assert self.grpc.flush()
    ...
    def test_set_option(self, key: str, value: str):
        assert self.grpc.set_option(key, value)

    @given(
        tx=binary()
    )
    def test_deliver_tx(self, tx: bytes):
        assert self.grpc.deliver_tx(tx)
    ...
    def test_check_tx(self, tx: bytes, is_new_check: bool):
        assert self.grpc.check_tx(tx, is_new_check)
    ...
    def test_query(self, data: bytes, path: str, height: int, prove: bool):
        assert self.grpc.query(data, path, height, prove)

    def test_commit(self):
        assert self.grpc.commit()
    ...
    def test_init_chain(
        ...
        assert self.grpc.init_chain(
            ...
            evidence_max_bytes,
            ["ed25519" if pk else "secp256k1" for pk in pub_key_types][:min_validator_length],
            app_version,
            [(pk, "ed25519") if tp else (pk, "secp256k1") for pk, tp in validator_pub_keys][:min_validator_length],
    ...
    @given(
        ...
        last_commit_info_votes=lists(tuples(binary(), integers(INT_64_MIN_VALUE, INT_64_MAX_VALUE))),
    ...
    def test_begin_block(
        ...
        last_commit_len = min(last_commit_info_votes.__len__(), last_commit_info_signed_last_block.__len__())
        evidence_len = min(
    ...
    def test_end_block(self, height: int):
        assert self.grpc.end_block(height)

    def test_list_snapshots(self):
        assert self.grpc.list_snapshots()
    ...
    def test_offer_snapshot(
        ...
        assert self.grpc.offer_snapshot(height, format_, chunks, hash_, metadata, app_hash)
    ...
    def test_load_snapshot_chunk(self, height: int, format_: int, chunk: int):
        assert self.grpc.load_snapshot_chunk(height, format_, chunk)
    ...
    def test_apply_snapshot_chunk(self, index: int, chunk: bytes, sender: str):
        assert self.grpc.apply_snapshot_chunk(index, chunk, sender)
