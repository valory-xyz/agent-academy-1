...
class BaseFuzzyTests(TestCase, AEATestCaseMany):
    ...
    def setUpClass(cls) -> None:
        ...
        if hasattr(cls, "key_pairs"):
            Path(cls.current_agent_context, "ethereum_private_key.txt").write_text(
                cls.key_pairs[agent_id][1]  # type: ignore
            )
        else:
            cls.generate_private_key("ethereum", "ethereum_private_key.txt")
        cls.add_private_key("ethereum", "ethereum_private_key.txt")
        cls.set_config(
            "vendor.valory.connections.abci.config.use_grpc", cls.USE_GRPC
        )

        # we are mocking a tendermint node
        cls.set_config(
            "vendor.valory.connections.abci.config.use_tendermint", False
        )
