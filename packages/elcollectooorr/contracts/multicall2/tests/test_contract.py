...
"""Tests for elcollectoooorr/multicall2 contract."""
...
class TestTokenSettingsFactory(BaseGanacheContractTest):
    ...
    def test_aggregate(self) -> None:
        ...
        multicall_instance = Multicall2Contract.get_instance(self.ledger_api, self.contract_address)
        call = self.contract.encode_function_call(self.ledger_api, multicall_instance, fn_name="getEthBalance", args=[address],)
        # make the same call 3 times
        calls = [call, call, call]
        contract_address = cast(str, self.contract_address)
        results = self.contract.aggregate_and_decode(self.ledger_api, contract_address, calls)
